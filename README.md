...
- この Python には `feedparser`, `openai`, `playwright` などが入っており、加えてこのリポジトリでは `dotenv` import 互換モジュールを `src/dotenv/__init__.py` に置いているため、既存環境では import と `pytest` が通ります。
...
- `auto_note_pipeline.py` は `from dotenv import load_dotenv` を使う形に戻しています。
- ただし、この環境では `python-dotenv` パッケージ自体が未導入のため、リポジトリ内の `src/dotenv/__init__.py` で互換 API を提供しています。
- そのため、コードの利用側は通常どおり `load_dotenv()` を使えます。
...
- 記事生成プロンプトを、単なるニュース要約ではなく、**読者の未来が変わる感**と**伴走感**を出す方向へ変更しました。
- さらに、生成時の視点を `Market Analyst` / `Opportunity Designer` / `Note Writer` の3役に分け、誤解されやすいポイントの分解・市場機会・具体的な次の一歩を出しやすくしました。
